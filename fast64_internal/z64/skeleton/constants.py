...
# Extension hooks — HM64 registers callbacks here to add MM skeleton support.
# Upstream code calls these instead of directly accessing the enum list.
_skeleton_mode_items_callback = None
_skeleton_import_info_callback = None


def register_skeleton_extensions(mode_items_cb, import_info_cb):
    """Called by HM64 to register MM skeleton mode items and import info lookup."""
    global _skeleton_mode_items_callback, _skeleton_import_info_callback
    _skeleton_mode_items_callback = mode_items_cb
    _skeleton_import_info_callback = import_info_cb


def unregister_skeleton_extensions():
    global _skeleton_mode_items_callback, _skeleton_import_info_callback
    _skeleton_mode_items_callback = None
    _skeleton_import_info_callback = None


def get_skeleton_mode_items(self, context) -> list:
    if _skeleton_mode_items_callback:
        return _skeleton_mode_items_callback(self, context)
    return ootEnumSkeletonImportMode


def find_skeleton_import_info(mode: str, game_mode: str | None = None):
    if _skeleton_import_info_callback:
        result = _skeleton_import_info_callback(mode, game_mode)
        if result is not None:
            return result
    return ootSkeletonImportDict.get(mode)
