...
def otr_header(resource_type: int):
    # byte order, is custom, 2 unused, type, version, id
    data = bytearray(struct.pack("<BBBBIIQ", 0, 1, 0, 0, resource_type, 0, OTR_ID))
...
def _write_texture_resource(otex_format: str, width: int, height: int, pixels: bytes):
    """u32 type, u32 width, u32 height, u32 byte count, then native N64 pixels"""
    data = otr_header(RT_TEXTURE)
...
def _collect_textures(
    ...
    for key, fImage in fModel.textures.items():
        ...
        if embed_images and otex_format not in BIN_TEX_FORMATS:
            raise PluginError(
                f"Texture '{fImage.name}' is {otex_format}. A .bin has no bit for I or IA. Use "
                "RGBA16, RGBA32, CI4 or CI8."
            )
    ...
    for key, fImage in fModel.textures.items():
        ...
        if embed_images:
            ...
            resources.append(_write_texture_resource(otex_format, fImage.width, fImage.height, pixels))
...
def _image_folds(mesh_objects, shade_by_material):
    ...
    for material, f3d_mat in _f3d_materials(mesh_objects):
        ...
        for tex_slot in (f3d_mat.tex0, f3d_mat.tex1):
            if tex_slot.tex is not None and tex_slot.tex_set:
